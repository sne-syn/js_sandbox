...
function f10() {
    ...
    for (let i = 0; i < a10.length; i++) {
        for (let k = 0; k < a10[i].length; k++) {
            (typeof a10[i][k] === 'string') ? out += a10[i][k] + ' ': out;
        }
    }
...
function f15() {
    ...
    for (let i = 0; i < a15.length; i++) {
        maxLength = (maxLength < a15[i].length) ? a15[i].length : maxLength;
    }
